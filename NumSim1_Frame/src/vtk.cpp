...
#include "comm.hpp"//"communicator.hpp"
#include "vtk.hpp"
#include <cstring>
#include <cstdio>
//------------------------------------------------------------------------------
uint32_t VTK::_cnt = 0;
//------------------------------------------------------------------------------
VTK::VTK(const multi_real_t &h, const multi_index_t &fieldsize,
         const multi_index_t &globalFieldSize, const int &rank, const int &size,
         const multi_index_t &fieldDims)
    : _h(h), _fieldsize(fieldsize), _globalFieldSize(globalFieldSize),
      _fieldDims(fieldDims), _rank(rank), _size(size) {
...
VTK::VTK(const multi_real_t &h, const multi_index_t &fieldsize,
    ...
    : _h(h), _fieldsize(fieldsize), _globalFieldSize(globalFieldSize),
      _fieldDims(fieldDims), _offset(offset), _rank(rank), _size(size) {
  _handle = NULL;
  _masterHandle = NULL;
}
