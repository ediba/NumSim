...
int main(int argc, char **argv) {
  ...
  RedOrBlackSOR redOrBlackSOR(&geom, param.Omega(), &comm);
  MPI_Barrier(MPI_COMM_WORLD);
    ...
    while (run) {
        ...
        if (key == 10) {
            ...
            redOrBlackSOR.Cycle(&testgrid,&zerogrid); //TODO: den Teil kann man intelligenter gestalten, indem evtl. alle auf einen Tastendruck reagieren. im Moment muss jedes Fenster einzeln gedrückt werden
        }
        if (key == -1) {
            run = false;
        }
